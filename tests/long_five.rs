...
fn all_binaries() -> Result<(), Box<dyn Error>> {
    // A
    let (thread_a_to_b, s_a_to_b): (JoinHandle<()>, BtoA) = fork_with_thread_id(binary_a_to_b);
    let (thread_a_to_c, s_a_to_c): (JoinHandle<()>, CtoA) = fork_with_thread_id(binary_a_to_c);
    let (thread_a_to_d, s_a_to_d): (JoinHandle<()>, DtoA) = fork_with_thread_id(binary_a_to_d);
    let (thread_a_to_e, s_a_to_e): (JoinHandle<()>, EtoA) = fork_with_thread_id(binary_a_to_e);
    // B
    let (thread_b_to_c, s_b_to_c): (JoinHandle<()>, CtoB) = fork_with_thread_id(binary_b_to_c);
    let (thread_b_to_d, s_b_to_d): (JoinHandle<()>, DtoB) = fork_with_thread_id(binary_b_to_d);
    let (thread_b_to_e, s_b_to_e): (JoinHandle<()>, EtoB) = fork_with_thread_id(binary_b_to_e);
    // C
    let (thread_c_to_d, s_c_to_d): (JoinHandle<()>, CtoD) = fork_with_thread_id(binary_d_to_c);
    let (thread_c_to_e, s_c_to_e): (JoinHandle<()>, CtoE) = fork_with_thread_id(binary_e_to_c);
    // D
    let (thread_d_to_e, s_d_to_e): (JoinHandle<()>, DtoE) = fork_with_thread_id(binary_e_to_d);

    binary_b_to_a(s_a_to_b).unwrap();
    binary_c_to_a(s_a_to_c).unwrap();
    binary_d_to_a(s_a_to_d).unwrap();
    binary_e_to_a(s_a_to_e).unwrap();

    binary_c_to_b(s_b_to_c).unwrap();
    binary_d_to_b(s_b_to_d).unwrap();
    binary_e_to_b(s_b_to_e).unwrap();

    binary_c_to_d(s_c_to_d).unwrap();
    binary_c_to_e(s_c_to_e).unwrap();

    binary_d_to_e(s_d_to_e).unwrap();
