        ...
        if compile_file in d and 'timed' not in d and 'checking' not in d and 'transport' not in d:
            try:
                ...
                with open(result_folder / result_file, 'a') as report_file:
                    ...
                    bar_running_baking[index_compile[compile_file]] = bench[compile_file]/1000000
                    report_file.write('; ')
                    report_file.write('\n')
            ...
            try:
                ...
                with open(result_folder / result_file, 'a') as report_file:
                    ...
                    bar_running_timed[index_compile[compile_file + '_timed']] = bench[compile_file + '_timed']/1000000
                    report_file.write('; ')
                    report_file.write('\n')
...
ax.bar(np.arange(len(bar_running_baking)) -  width/2, bar_running_baking, width=width, color='#d62728')
ax.bar(np.arange(len(bar_running_timed)) + width/2, bar_running_timed, width=width, color='#9467bd')

ax.set_title("Running time", pad=10, fontsize=30)
...
index_graphs = 0
while os.path.isfile('graphs_bench/examples/graphs_examples_bench_' + str(index_graphs) + '.pdf'):
    index_graphs += 1

name_graph = './graphs_bench/examples/graphs_examples_bench_' + str(index_graphs) + '.pdf'
