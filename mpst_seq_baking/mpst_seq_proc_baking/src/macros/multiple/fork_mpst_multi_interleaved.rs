...
#[derive(Debug)]
pub struct ForkMPSTMultiInterleavedMacroInput {
    func_name: syn::Ident,
...
impl From<ForkMPSTMultiInterleavedMacroInput> for proc_macro2::TokenStream {
    fn from(input: ForkMPSTMultiInterleavedMacroInput) -> proc_macro2::TokenStream {
        input.expand()
    }
}

impl ForkMPSTMultiInterleavedMacroInput {
    ...
    fn expand(&self) -> proc_macro2::TokenStream {
        ...
        let roles: Vec<proc_macro2::TokenStream> = (1..=self.nsessions)
            .map(|i| {
                let temp_ident =
        ...
        let roles_struct: Vec<proc_macro2::TokenStream> = (1..=self.nsessions)
            .map(|i| {
                let temp_ident =
                    syn::Ident::new(&format!("R{}", i), proc_macro2::Span::call_site());
                quote! {
        ...
        let names: Vec<proc_macro2::TokenStream> = (1..=self.nsessions)
            .map(|i| {
                let temp_ident =
        ...
        let names_struct: Vec<proc_macro2::TokenStream> = (1..=self.nsessions)
            .map(|i| {
                let temp_ident =
                    syn::Ident::new(&format!("N{}", i), proc_macro2::Span::call_site());
                quote! {
