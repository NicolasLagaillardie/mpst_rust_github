...
pub(crate) fn fn_previous_message_wrt_clocks(
    ...
    if let Some(role_time_bounds) = previous_message_wrt_clocks.get_mut(&elts.sender) {
        if let Some(previous_time_bound) = role_time_bounds.remove(&elts.clock) {
            println!(
                "{} / {} / {} / {} / {} / {}",
                elts.left_bound,
                elts.left_bracket,
                elts.right_bound,
                elts.right_bracket,
                previous_time_bound.0,
                previous_time_bound.1
            );
        ...
        previous_message_wrt_clocks.insert(elts.sender.to_string(), temp_time_bound);
    }

    if let Some(role_time_bounds) = previous_message_wrt_clocks.get_mut(&elts.receiver) {
        if let Some(previous_time_bound) = role_time_bounds.remove(&elts.clock) {
            println!(
                "{} / {} / {} / {} / {} / {}",
                elts.left_bound,
