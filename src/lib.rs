...
//!     their name.
//! 3. `macros_multiple`: feature for implementing protocols with any number of
//!     participants. Contains `macros_simple`.
//! 4. `baking`: feature for implementing protocols with any
//!     number of participants and using associated functions instead of functions. Contains
//!     `macros_multiple`.
//! 5. `transport_tcp`: feature containing primitives for communicating with TCP.
...
//!     `openssl`, `pkg-config` and `libssl-dev` installed on your machine**.
//! 7. `transport_http`: feature containing primitives for communicating with HTTP/HTTPS. **Requires `openssl`,
//!     `pkg-config` and `libssl-dev` installed on your machine**.
