...
use crate::common_functions::expand::fork::fork_atmp_interleaved_mpst;
...
#[derive(Debug)]
pub(crate) struct BakingATMPInterleavedWithEnumAndCancel {
...
impl Parse for BakingATMPInterleavedWithEnumAndCancel {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        Ok(BakingATMPInterleavedWithEnumAndCancel {
...
impl From<BakingATMPInterleavedWithEnumAndCancel> for TokenStream {
    fn from(input: BakingATMPInterleavedWithEnumAndCancel) -> TokenStream {
        input.expand()
    }
}

impl BakingATMPInterleavedWithEnumAndCancel {
    fn expand(&self) -> TokenStream {
        ...
        let roles_struct_one = create_atmp_role_structs(&self.all_roles_one);
        ...
        let close_methods_one: TokenStream =
            close_atmp(&self.meshedchannels_name_one, self.number_roles_one);

        let choose_mpst_create_multi_to_all_one = choose_atmp_mpst_create_multi_to_all(
            &self.meshedchannels_name_one,
            &self.all_roles_one,
            self.number_roles_one,
        );
        ...
        let roles_struct_two = create_atmp_role_structs(&self.all_roles_two);
        ...
        let close_methods_two: TokenStream =
            close_atmp(&self.meshedchannels_name_two, self.number_roles_two);

        let choose_mpst_create_multi_to_all_two = choose_atmp_mpst_create_multi_to_all(
            &self.meshedchannels_name_two,
            &self.all_roles_two,
            self.number_roles_two,
        );
        ...
        let quote_fork_mpst = fork_atmp_interleaved_mpst(
            &self.func_name,
            &self.meshedchannels_name_one,
            self.number_roles_one,
