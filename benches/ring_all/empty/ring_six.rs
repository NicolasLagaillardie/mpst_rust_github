...
use mpstthree::{
    ...
    create_send_mpst_session_bundle, offer, offer_mpst,create_multiple_normal_name_short
};
...
pub fn ring_protocol_mpst(c: &mut Criterion) {
    c.bench_function(&format!("ring six empty protocol MPST {}", LOOPS), |b| {
        b.iter(all_mpst)
    });
}

pub fn ring_protocol_binary(c: &mut Criterion) {
    c.bench_function(&format!("ring six empty protocol binary {}", LOOPS), |b| {
        b.iter(all_binaries)
    });
}
...
pub fn ring_protocol_crossbeam(c: &mut Criterion) {
    c.bench_function(
        &format!("ring six empty protocol crossbeam {}", LOOPS),
