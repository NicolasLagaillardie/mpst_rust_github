...
pub fn ring_protocol_mpst(c: &mut Criterion) {
    c.bench_function(&format!("ring eleven empty protocol MPST {}", LOOPS), |b| {
        b.iter(all_mpst)
    });
}
...
pub fn ring_protocol_binary(c: &mut Criterion) {
    c.bench_function(
        &format!("ring eleven empty protocol binary {}", LOOPS),
...
pub fn ring_protocol_crossbeam(c: &mut Criterion) {
    c.bench_function(
        &format!("ring eleven empty protocol crossbeam {}", LOOPS),
