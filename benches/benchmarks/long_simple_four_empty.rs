...
fn all_mpst() -> Result<(), Box<dyn std::any::Any + std::marker::Send>> {
    let (thread_a, thread_b, thread_c, thread_d) = fork_mpst(
        black_box(simple_four_endpoint_a),
        black_box(simple_four_endpoint_b),
        black_box(simple_four_endpoint_c),
        black_box(simple_four_endpoint_d),
    );
...
fn long_simple_protocol_mpst(c: &mut Criterion) {
    c.bench_function(
        &format!("long four empty simple protocol MPST {}", SIZE),
        |b| b.iter(|| all_mpst()),
    );
}
...
fn long_simple_protocol_binary(c: &mut Criterion) {
    c.bench_function(
        &format!("long four empty simple protocol binary {}", SIZE),
...
fn long_simple_protocol_crossbeam(c: &mut Criterion) {
    c.bench_function(
        &format!("long four empty simple protocol crossbeam {}", SIZE),
