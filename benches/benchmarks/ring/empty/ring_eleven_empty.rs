...
fn ring_protocol_mpst(c: &mut Criterion) {
    c.bench_function(&format!("ring eleven empty protocol MPST {}", SIZE), |b| {
        b.iter(|| all_mpst())
    });
}
...
fn ring_protocol_binary(c: &mut Criterion) {
    c.bench_function(
        &format!("ring eleven empty protocol binary {}", SIZE),
...
fn ring_protocol_crossbeam(c: &mut Criterion) {
    c.bench_function(
        &format!("ring eleven empty protocol crossbeam {}", SIZE),
