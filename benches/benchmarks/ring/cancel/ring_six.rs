...
bundle_struct_fork_close_multi_cancel!(close_mpst_multi, fork_mpst, SessionMpstSix, 6);
...
fn ring_six_binary(c: &mut Criterion) {
    c.bench_function(&format!("ring six cancel protocol binary {}", SIZE), |b| {
        b.iter(|| all_binaries())
    });
}
...
fn ring_six_crossbeam(c: &mut Criterion) {
    c.bench_function(
        &format!("ring six cancel protocol crossbeam {}", SIZE),
