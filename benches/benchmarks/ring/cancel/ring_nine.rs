...
type EndpointForwardI = SessionMpstNine<
    Choose0fromItoA,
    Choose0fromItoB,
    Choose0fromItoC,
    Choose0fromItoD,
    ...
    NameI,
>;
type EndpointBackwardI = SessionMpstNine<
    Choose0fromItoA,
    Choose0fromItoB,
    Choose0fromItoC,
    Choose0fromItoD,
...
fn ring_protocol_binary(c: &mut Criterion) {
    c.bench_function(&format!("ring nine cancel protocol binary {}", SIZE), |b| {
        b.iter(|| all_binaries())
    });
}
...
fn ring_protocol_crossbeam(c: &mut Criterion) {
    c.bench_function(
        &format!("ring nine cancel protocol crossbeam {}", SIZE),
