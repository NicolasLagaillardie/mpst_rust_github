...
type EndpointForwardJ = SessionMpstTen<
    Choose0fromJtoA,
    Choose0fromJtoB,
    Choose0fromJtoC,
    Choose0fromJtoD,
    ...
    NameJ,
>;
type EndpointBackwardJ = SessionMpstTen<
    Choose0fromJtoA,
    Choose0fromJtoB,
    Choose0fromJtoC,
    Choose0fromJtoD,
...
fn ring_protocol_mpst(c: &mut Criterion) {
    c.bench_function(&format!("ring ten cancel protocol MPST {}", SIZE), |b| {
        b.iter(|| all_mpst())
    });
}

fn ring_protocol_binary(c: &mut Criterion) {
    c.bench_function(&format!("ring ten cancel protocol binary {}", SIZE), |b| {
        b.iter(|| all_binaries())
    });
}
...
fn ring_protocol_crossbeam(c: &mut Criterion) {
    c.bench_function(
        &format!("ring ten cancel protocol crossbeam {}", SIZE),
