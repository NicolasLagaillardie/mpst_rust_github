...
type EndpointForwardH = SessionMpstEight<
    Choose0fromHtoA,
    Choose0fromHtoB,
    Choose0fromHtoC,
    Choose0fromHtoD,
    ...
    NameH,
>;
type EndpointBackwardH = SessionMpstEight<
    Choose0fromHtoA,
    Choose0fromHtoB,
    Choose0fromHtoC,
    Choose0fromHtoD,
...
fn ring_protocol_mpst(c: &mut Criterion) {
    c.bench_function(&format!("ring eight cancel protocol MPST {}", SIZE), |b| {
        b.iter(|| all_mpst())
    });
}
...
fn ring_protocol_binary(c: &mut Criterion) {
    c.bench_function(
        &format!("ring eight cancel protocol binary {}", SIZE),
...
fn ring_protocol_crossbeam(c: &mut Criterion) {
    c.bench_function(
        &format!("ring eight cancel protocol crossbeam {}", SIZE),
