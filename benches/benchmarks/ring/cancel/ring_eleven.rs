...
bundle_struct_fork_close_multi_cancel!(close_mpst_multi, fork_mpst, SessionMpstEleven, 11);
...
type EndpointForwardK = SessionMpstEleven<
    Choose0fromKtoA,
    Choose0fromKtoB,
    Choose0fromKtoC,
    Choose0fromKtoD,
    ...
    NameK,
>;
type EndpointBackwardK = SessionMpstEleven<
    Choose0fromKtoA,
    Choose0fromKtoB,
    Choose0fromKtoC,
    Choose0fromKtoD,
...
fn ring_protocol_mpst(c: &mut Criterion) {
    c.bench_function(&format!("ring eleven cancel protocol MPST {}", SIZE), |b| {
        b.iter(|| all_mpst())
    });
}
...
fn ring_protocol_binary(c: &mut Criterion) {
    c.bench_function(
        &format!("ring eleven cancel protocol binary {}", SIZE),
...
fn ring_protocol_crossbeam(c: &mut Criterion) {
    c.bench_function(
        &format!("ring eleven cancel protocol crossbeam {}", SIZE),
