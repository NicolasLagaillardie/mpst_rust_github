...
create_recv_mpst_session_bundle!(
    recv_start_api_from_controller, Controller, 1 |
    recv_response_api_from_storage, Storage, 3 | =>
    Api, SessionMpstFour, 4
);
create_recv_mpst_session_bundle!(
    recv_new_status_controller_from_storage, Storage, 3 | =>
    Controller, SessionMpstFour, 4
);
create_recv_mpst_session_bundle!(
    recv_logs_from_storage, Storage, 3 | =>
    Logs, SessionMpstFour, 4
);
create_recv_mpst_session_bundle!(
    recv_request_storage_from_api, Api, 1 |
    recv_start_storage_from_controller, Controller, 2 | =>
    Storage, SessionMpstFour, 4
);
...
fn actyx_os(c: &mut Criterion) {
    c.bench_function(&format!("Actyx OS 2"), |b| b.iter(|| all_mpst()));
}
