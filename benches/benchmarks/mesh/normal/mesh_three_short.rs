...
fn mesh_protocol_mpst(c: &mut Criterion) {
    c.bench_function(&format!("mesh three protocol MPST  short {}", LOOPS), |b| {
        b.iter(|| all_mpst())
    });
}
...
fn mesh_protocol_binary(c: &mut Criterion) {
    c.bench_function(
        &format!("mesh three protocol binary  short {}", LOOPS),
...
fn mesh_protocol_crossbeam(c: &mut Criterion) {
    c.bench_function(
        &format!("mesh three protocol crossbeam short {}", LOOPS),
