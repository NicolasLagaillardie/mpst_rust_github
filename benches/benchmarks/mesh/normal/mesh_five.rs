...
fn mesh_protocol_mpst(c: &mut Criterion) {
    c.bench_function(&format!("mesh five protocol MPST {}", SIZE), |b| {
        b.iter(|| all_mpst())
    });
}

fn mesh_protocol_binary(c: &mut Criterion) {
    c.bench_function(&format!("mesh five protocol binary {}", SIZE), |b| {
        b.iter(|| all_binaries())
    });
}

fn mesh_protocol_crossbeam(c: &mut Criterion) {
    c.bench_function(&format!("mesh five protocol crossbeam {}", SIZE), |b| {
