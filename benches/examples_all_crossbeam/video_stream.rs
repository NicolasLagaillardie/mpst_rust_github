...
type S0 = Receiver<Receiver<Receiver<Receiver<Receiver<Receiver<()>>>>>>;
type S1 = Receiver<Receiver<Receiver<Receiver<Receiver<()>>>>>;
type S2 = Receiver<Receiver<Receiver<Receiver<()>>>>;
type S3 = Receiver<Receiver<Receiver<()>>>;
type S4 = Receiver<Receiver<()>>;
type S5 = Receiver<()>;
type S6 = ();
...
fn aux() {
    let main = spawn(move || {
        for _ in 0..LOOPS {
            ...
            sender_s_6.send(()).unwrap();
            receiver_s_6_bis.recv().unwrap();
        }
    });
...
pub fn video_stream_crossbeam(c: &mut Criterion) {
    c.bench_function("Video stream crossbeam", |b| b.iter(aux));
}
...
criterion_group! {
    ...
    targets = video_stream_crossbeam,
}
