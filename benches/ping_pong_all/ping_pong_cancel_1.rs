...
use mpstthree::{
    bundle_struct_fork_close_multi_cancel, choose, choose_mpst_multi_to_all,
...
fn ping_pong_protocol_binary(c: &mut Criterion) {
    c.bench_function(&format!("ping pong cancel protocol binary {}", SIZE), |b| {
        b.iter(|| all_binaries())
    });
}
...
fn ping_pong_protocol_crossbeam(c: &mut Criterion) {
    c.bench_function(
        &format!("ping pong cancel protocol crossbeam {}", SIZE),
