...
#[derive(Debug)]
pub struct ChooseTypeMultiCancelToAllMacroInput {
...
impl Parse for ChooseTypeMultiCancelToAllMacroInput {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        let nsessions = all_receivers.len().to_string().parse::<u64>().unwrap() + 2;
        ...
        Ok(ChooseTypeMultiCancelToAllMacroInput {
...
impl From<ChooseTypeMultiCancelToAllMacroInput> for proc_macro2::TokenStream {
    fn from(input: ChooseTypeMultiCancelToAllMacroInput) -> proc_macro2::TokenStream {
        input.expand()
    }
}

impl ChooseTypeMultiCancelToAllMacroInput {
    ...
    fn expand(&self) -> proc_macro2::TokenStream {
        ...
        let new_roles: Vec<proc_macro2::TokenStream> = (2..=self.nsessions)
            .map(|i| {
                let temp_ident =
        ...
        let new_names: Vec<proc_macro2::TokenStream> = (2..self.nsessions)
            .map(|i| {
                let temp_name =
        ...
        let all_send: Vec<proc_macro2::TokenStream> = (2..self.nsessions)
            .map(|i| {
