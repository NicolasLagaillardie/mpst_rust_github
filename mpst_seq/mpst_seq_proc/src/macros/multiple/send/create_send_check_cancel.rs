...
#[derive(Debug)]
pub struct CreateSendCheckCancelMacroInput {
    func_name: syn::Ident,
    receiver: syn::Ident,
    sender: syn::Ident,
...
impl Parse for CreateSendCheckCancelMacroInput {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        Ok(CreateSendCheckCancelMacroInput {
            func_name,
            receiver,
            sender,
            meshedchannels_name,
...
impl From<CreateSendCheckCancelMacroInput> for proc_macro2::TokenStream {
    fn from(input: CreateSendCheckCancelMacroInput) -> proc_macro2::TokenStream {
        input.expand()
    }
}

impl CreateSendCheckCancelMacroInput {
    fn expand(&self) -> proc_macro2::TokenStream {
        ...
        let session_types_struct: Vec<proc_macro2::TokenStream> = (2..self.nsessions)
            .map(|i| {
                let temp_ident =
                    syn::Ident::new(&format!("S{}", i), proc_macro2::Span::call_site());
                quote! {
        ...
        let send_types: Vec<proc_macro2::TokenStream> = (2..self.nsessions)
            .map(|i| {
                let temp_ident =
                    syn::Ident::new(&format!("S{}", i), proc_macro2::Span::call_site());
                if i == self.exclusion {
